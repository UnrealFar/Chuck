...
class Fun(commands.Cog):
    ...
    async def get_rock(
        ...
        async with self.session.get(
            ...
            rok = await d.json()
            em = discord.Embed(title=rok["name"], description=rok["description"])
    ...
    async def get_plane(self, i: discord.Interaction):
        ...
        await i.response.defer()
        async with self.session.get(self.api_urls["plane"] + "/planes/random") as d:
            if d.status != 200:
                return await i.edit_original_message(
                    content=f"Uh oh! PlaneAPI responded with status {d.status}. Please wait while this endpoint is fixed!"
                )
            plane = await d.json()
            em = discord.Embed(

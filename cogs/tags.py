...
class TCreateM(ui.Modal, title = "Create a tag"):
    ...
    content = ui.TextInput(label='Content of the tag', style = discord.TextStyle.paragraph)
    ...
    async def on_submit(self, i: discord.Interaction):
        ...
        async with db.execute(
            ...
            if int(cd[0]) != i.user.id:
                return await i.response.send_message("Only the owner of the tag can edit a tag!")
        async with db.execute(
            """UPDATE tags
            SET content = ?, created = ?
            WHERE g_id = ? AND name = ?
            """,
            (content, discord.utils.utcnow(), self.g_id, name,)
        ): 
            await db.commit()
            await i.response.send_message(
                f"Edited tag **{name}**!"
...
class Tags(commands.Cog):
    ...
    tag_group = app_commands.Group(name = "tag", description = "Commands related to tag system.")
    ...
    async def tcmd(
        ...
        async with self.db.execute(
            ...
            if not td:
                return await i.response.send_message("A tag with that name does not exist!")
            await i.response.send_message(td[0] if raw is False else f"```\n{td[0]}\n```")
    ...
    async def tcreate(
        ...
        await i.response.send_modal(
            TCreateM(self, i.guild_id, i.user.id)
        )
    ...
    async def tedit(
        ...
        await i.response.send_modal(
            TEditM(self, i.guild_id, i.user.id)
        )
    ...
    async def tdelete(
        ...
        async with self.db.execute(
            ...
            if not d:
                return await i.response.send_message("A tag with this name does not exist!")
            if int(d[0]) != i.user.id or not i.user.guild_permissions.administrator:
                return await i.response.send_message("You cannot delete this tag as you are neither its owner or a guild administrator!")
        async with self.db.execute(
            """DELETE FROM tags
            WHERE g_id = ? AND name = ?
            """,

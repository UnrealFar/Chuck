...
class Player:
    ...
    def dp_event(self, dp_ev_name: str, *args, **kwargs):
    ...
    async def fetch_song(self, song: str):
        ...
        fa = (
            (", " + data["featuredArtists"]) if len(data["featuredArtists"]) > 1 else ""
        )
    ...
    def play(self) -> Song:
        ...
        src = discord.PCMVolumeTransformer(
            discord.FFmpegPCMAudio(
                song.source, options="-vn -loglevel panic", before_options="-nostdin"
            )
        )
        self.vc.play(src, after=lambda _: self.dp_event("stop"))
        self.playing = True
        return song
...
class Music(commands.Cog):
    ...
    async def cog_load(self) -> None:
        ...
        async with aiosqlite.connect("db/music.sqlite") as self.db:
    ...
    async def skip_command(self, i: discord.Interaction):
        """Skip the current playing song."""
        player = await self.create_player(i)
        if not player:
            return await i.response.send_message(
                "Please start playing some music first!"
            )
